...
$(function(){
    let myClientHeight = document.body.clientHeight;
    window.onresize = function () {
        myClientHeight = document.body.clientHeight;
        console.log('myClientHeight',myClientHeight)
    }
    let chart21W = $('#chart2-1').width();
    let myChartH = 0;
    if ((chart21W * 1.3) > (myClientHeight / 2 - 192)) {
        myChartH = myClientHeight / 2 - 192;
    } else {
        myChartH = chart21W * 1.3;
    }

    $('#chart2-1').css("height", myChartH + 'px')
    $('#chart2-2').css("height", myChartH + 'px')
    $('#chart2-3').css("height", myChartH + 'px')
    $('#chart2-4').css("height", myChartH + 'px')
    ...
    let countCityChart = null;
    let countCityOption = null;
    let countTaskByMonth_setInterval = null;
    let chartLegend_setInterval1 = null;
    let chartLegend_setInterval2 = null;
    let chartLegend_setInterval3 = null;
    let chartLegend_setInterval4 = null;
    let countCourt_setInterval1 = null;
    let countCourt_setInterval2 = null;

    let countCourtChart = null;
    let myChart1 = null;
    let myChart2 = null;
    let myChart3 = null;
    let myChart4 = null;
    ...
    setInterval(function(){
        ...
        myChart3.clear(); 
        myChart4.clear();
        countCityChart.clear();
        myChart5.clear(); 
        $(".dataNums").remove();
        clearInterval(countTaskByMonth_setInterval);
        clearInterval(chartLegend_setInterval1);
        clearInterval(chartLegend_setInterval2);
        clearInterval(chartLegend_setInterval3);
        clearInterval(chartLegend_setInterval4);
        clearInterval(countCourt_setInterval1);
        clearInterval(countCourt_setInterval2);

        pretreatment();
    },30000)

    //标的物分类占比情况定时循环显示
    function chartLegendShow (element,myChart,chartLegend_setInterval) {
        let chartLegend = element.length;
        let chartLegend_index = 0;
        chartLegend_setInterval = setInterval(chartLegendF,3750);
        let chartLegend_color = ['#3BD6B8','#EECB6C','#E8928D'];
        let frequency = 1;
        function chartLegendF(){
            if (chartLegend_index > 0) {
                myChart.dispatchAction({type:'hideTip',seriesIndex: chartLegend_index + 1,dataIndex: 1});
                myChart.dispatchAction({type: 'downplay',seriesIndex: chartLegend_index + 1,dataIndex: 1});
                element.find('.chart-legend-icon').eq(chartLegend_index-1).css('background','#5E5F63');
                element.eq(chartLegend_index-1).css('color','#5E5F63');
            }
            if (chartLegend_index === chartLegend) {
                if (frequency === 2) {
                    clearInterval(chartLegend_setInterval);
                } else {
                    chartLegend_index = 0; 
                }
                frequency++;
            ...
            element.eq(chartLegend_index).css('color',chartLegend_color[chartLegend_index])
            element.find('.chart-legend-icon').eq(chartLegend_index).css('background',chartLegend_color[chartLegend_index]);
            myChart.dispatchAction({type:'showTip',seriesIndex: chartLegend_index + 2,dataIndex: 1});
            myChart.dispatchAction({type: 'highlight',seriesIndex: chartLegend_index + 2,dataIndex: 1});
            chartLegend_index++;
        }
    }
    ...
    function countCourtGet() {
        ...
        httpGet( api.countCourt,{},
            function success(result){
                ...
                $('.court-total').html('总共'+totalNum+'家');
    ...
    function drawChart6(dataInit,inquestList,lookList) {
        ...
        let countTaskByMonth_count = 0;
        let  countTaskByMonth_total = dataInit.length; // series.data.length
        function autoTip() {
            countTaskByMonth_setInterval = setInterval(function() {
                let curr = countTaskByMonth_count % countTaskByMonth_total;
                myChart5.dispatchAction({
                    type: 'showTip',
                    seriesIndex: 0, // 因为只有一组数据，所以此处应为0
                    dataIndex: curr
                });
                countTaskByMonth_count += 1;
            }, 2700);
        }
        autoTip();
    ...
    function drawChart5(dataInit) {
        // 基于准备好的dom，初始化echarts实例
        countCourtChart = echarts.init(document.getElementById('chart3'))
        // 监听屏幕变化自动缩放图表
        window.addEventListener('resize', function () {
            countCourtChart.resize()
        })
        // 绘制图表
        countCourtChart.setOption({ // 进行相关配置
            // 提示框组件
            tooltip: {
            ...
            let total = 3; // series.data.length
            let count = 0;
            let count1 = -1;
            function autoTip() {
                countCourt_setInterval1 = setInterval(function() {
                    let curr = count % total;
                    countCourtChart.dispatchAction({
                        type: 'mapSelect',
                        seriesIndex: 0, // 因为只有一组数据，所以此处应为0
                        dataIndex: curr
                    });
                ...
                countCourt_setInterval2 = setInterval(function() {
                    let curr1 = count1 % total;
    ...
    function drawChart1(dataInit1) {
        ...
        chartLegendShow($('.chart1-legend'),myChart1,chartLegend_setInterval1);
    }
    //自定义图例高亮时
    ...
    function drawChart2(dataInit2) {
        ...
        chartLegendShow($('.chart2-legend'),myChart2,chartLegend_setInterval2);
    ...
    function drawChart3(dataInit3) {
        ...
        chartLegendShow($('.chart3-legend'),myChart3,chartLegend_setInterval3);
    ...
    function drawChart4(dataInit4) {
        ...
        chartLegendShow($('.chart4-legend'),myChart4,chartLegend_setInterval4);
    ...
    function topTableShow(topList,totalNum){
        ...
        $('.wrapper-body .table-body').html(contentShow);
        let tableBadyH = $('.wrapper-body .table-body').height();
        if (tableBadyH > (myClientHeight/2-240)) {
            $('.wrapper-body .table-body').css({'maxHeight':'calc(50vh - 15rem)','overflowY':'scroll'});
        }
        setTimeout(function(){
    ...
    function drawChart(dataInit,dataSource) {
        // 基于准备好的dom，初始化echarts实例
        countCityChart = echarts.init(document.getElementById('chart'))
        // 监听屏幕变化自动缩放图表
        window.addEventListener('resize', function () {
            countCityChart.resize()
        })
        // 绘制图表
        countCityChart.setOption({
            // 图表主标题
            title: {
                text: '', // 主标题文本，支持使用 \n 换行
        ...
        // countCityOption = countCityChart.getOption();
    }
